...
class HFImageCaptioner:
    def __init__(self, model_name: str, max_new_tokens: int = 24) -> None:
        ...
        self._pipeline = pipeline("image-text-to-text", model=model_name)
        self._Image = Image
        self._np = np
...
def _extract_generated_text(raw: Any) -> str:
    if isinstance(raw, list) and raw:
        ...
        if isinstance(item, dict):
            return str(item.get("generated_text") or item.get("caption") or "")
        return str(item)
    if isinstance(raw, dict):
        return str(raw.get("generated_text") or raw.get("caption") or "")
    return str(raw or "")
